...
describe('transformPool', () => {
  // Transform pool object with the unity id for a label. For display purposes only.
  const poolTable: [number, PoolConfig][] = poolsConfig.map((poolsConfigItem) => [
    poolsConfigItem.unityId,
    poolsConfigItem,
  ])
  it.each(poolTable)('transforms pool %d correctly', (unityId, config) => {
    const pool = {
      ...config,
    ...
    expect(transformedPool).toHaveProperty('unityId', unityId)
