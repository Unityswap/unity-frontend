...
export const usePoolFromPid = (unityId: number): Pool => {
  const pool = useSelector((state: State) => state.pools.data.find((p) => p.unityId === unityId))
...
export const usePriceRyipBusd = (): BigNumber => {
  const ryipBnbFarm = useFarmFromPid(1)
  const bnbBusdPrice = usePriceBnbBusd()

  const ryipBusdPrice = ryipBnbFarm.tokenPriceVsQuote ? bnbBusdPrice.times(ryipBnbFarm.tokenPriceVsQuote) : BIG_ZERO

  return ryipBusdPrice
}
