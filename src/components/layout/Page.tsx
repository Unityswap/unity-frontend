...
const PageMeta = () => {
  ...
  const ryipPriceUsdDisplay = ryipPriceUsd.eq(0)
    ? ''
    : `$${ryipPriceUsd.toNumber().toLocaleString(undefined, {
        minimumFractionDigits: 3,
        maximumFractionDigits: 3,
      })}`
  const pageMeta = customMeta[pathname] || {}
  const { title, description, image } = { ...DEFAULT_META, ...pageMeta }
  const pageTitle = ryipPriceUsdDisplay ? [title, ryipPriceUsdDisplay].join(' - ') : title
