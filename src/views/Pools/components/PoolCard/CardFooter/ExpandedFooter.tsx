...
import { getAddress, getRyipVaultAddress } from 'utils/addressHelpers'
...
const ExpandedFooter: React.FC<ExpandedFooterProps> = ({
  ...
  const ryipVaultContractAddress = getRyipVaultAddress()
          ...
          {totalStaked ? (
            <>
              <Balance
                ...
                value={
                  isAutoVault
                    ? getBalanceNumber(totalRyipInVault, stakingToken.decimals)
      ...
      {poolContractAddress && (
        <Flex mb="2px" justifyContent="flex-end">
          <LinkExternal
            bold={false}
            small
            href={`${BASE_BSC_SCAN_URL}/address/${isAutoVault ? ryipVaultContractAddress : poolContractAddress}`}
          >
            {t('View Contract')}
          </LinkExternal>
        </Flex>
