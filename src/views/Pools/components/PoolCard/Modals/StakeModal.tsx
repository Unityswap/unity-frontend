...
import { Modal, Text, Flex, Image, Button, Slider, BalanceInput, AutoRenewIcon, Link } from '@unityswap/uikit'
import { useTranslation } from 'contexts/Localization'
import { BASE_EXCHANGE_URL } from 'config'
import { useUnityStake } from 'hooks/useStake'
import { useUnityUnstake } from 'hooks/useUnstake'
...
const StakeModal: React.FC<StakeModalProps> = ({
  ...
  const { unityId, stakingToken, userData, stakingLimit, earningToken } = pool
  const { t } = useTranslation()
  const { theme } = useTheme()
  const { onStake } = useUnityStake(unityId, isBnbPool)
  const { onUnstake } = useUnityUnstake(unityId, pool.enableEmergencyWithdraw)
