...
import { Modal, Text, Flex, Image, Button, Slider, BalanceInput, AutoRenewIcon } from '@unityswap/uikit'
...
import useWithdrawalFeeTimer from 'hooks/ryipVault/useWithdrawalFeeTimer'
import { VaultFees } from 'hooks/ryipVault/useGetVaultFees'
import BigNumber from 'bignumber.js'
import { getFullDisplayBalance, formatNumber, getDecimalAmount } from 'utils/formatBalance'
import useToast from 'hooks/useToast'
import { Pool } from 'state/types'
import { VaultUser } from 'views/Pools/types'
import { convertRyipToShares } from '../../helpers'
...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    setPendingTx(true)
    const shareStakeToWithdraw = convertRyipToShares(convertedStakeAmount, pricePerFullShare)
    // trigger withdrawAll function if the withdrawal will leave 0.000001 RYIP or less
    const triggerWithdrawAllThreshold = new BigNumber(1000000000000)
    const sharesRemaining = userInfo.shares.minus(shareStakeToWithdraw.sharesAsBigNumber)
    const isWithdrawingAll = sharesRemaining.lte(triggerWithdrawAllThreshold)

    if (isWithdrawingAll) {
      ryipVaultContract.methods
        .withdrawAll()
        .send({ from: account })
        .on('sending', () => {
          setPendingTx(true)
