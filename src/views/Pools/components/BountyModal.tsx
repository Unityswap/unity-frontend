...
import { Modal, Text, Flex, Button, HelpIcon, AutoRenewIcon, useTooltip } from '@unityswap/uikit'
...
interface BountyModalProps {
  ryipBountyToDisplay: string
  dollarBountyToDisplay: string
  totalPendingRyipHarvest: BigNumber
  callFee: number
  onDismiss?: () => void
...
const BountyModal: React.FC<BountyModalProps> = ({
  ...
  const totalYieldToDisplay = getFullDisplayBalance(totalPendingRyipHarvest, 18, 3)
  ...
  const handleConfirmClick = async () => {
    ryipVaultContract.methods
      ...
      .on('receipt', () => {
        toastSuccess(t('Bounty collected!'), t('RYIP bounty has been sent to your wallet.'))
        setPendingTx(false)
        onDismiss()
      })
      .on('error', (error) => {
    ...
    <Modal title={t('Claim Bounty')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <Flex alignItems="flex-start" justifyContent="space-between">
        ...
        <Flex flexDirection="column">
          <Text bold>{ryipBountyToDisplay} RYIP</Text>
