...
export const useCompetitionRyipRewards = (userRyipReward: ReactText) => {
  const ryipAsBigNumber = new BigNumber(userRyipReward as string)
  const ryipBalance = getBalanceNumber(ryipAsBigNumber)
  const ryipPriceBusd = usePriceRyipBusd()
  return { ryipReward: ryipBalance, dollarValueOfRyipReward: ryipBalance * ryipPriceBusd.toNumber() }
}
