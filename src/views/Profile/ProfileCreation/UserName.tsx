...
const minimumRyipToRegister = new BigNumber(REGISTER_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)
...
const UserName: React.FC = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumRyipRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumRyipRequired = useHasRyipBalance(minimumRyipToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumRyipRequired={minimumRyipRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumRyipRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% RYIP is required', { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
