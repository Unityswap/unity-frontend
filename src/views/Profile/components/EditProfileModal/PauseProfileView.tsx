...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@unityswap/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberRyipToReactivate } = useGetProfileCosts()
  const { t } = useTranslation()
  const unityProfileContract = useProfileContract()
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {t(`Cost to reactivate in future: ${getBalanceNumber(numberRyipToReactivate)} RYIP`)}
      </Text>
