...
import { Button, Modal } from '@unityswap/uikit'
...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  const ryipCosts = (amount: string): number => {
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        max={fullBalance}
        symbol="TICKET"
        availableSymbol="RYIP"
      />
      <div>
        <Tips>{t(`1 Ticket = ${LOTTERY_TICKET_PRICE} RYIP`, { num: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your RYIP cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %amount% RYIP', { amount: ryipCosts(val) })}</Final>
      </div>
      <ModalActions>
