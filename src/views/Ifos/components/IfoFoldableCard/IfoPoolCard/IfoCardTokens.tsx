...
import { Text, Flex, Box, Image, CheckmarkCircleIcon, FlexProps, HelpIcon, useTooltip } from '@unityswap/uikit'
...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const renderTokenSection = () => {
    ...
    if (account && !hasProfile) {
      return <Text textAlign="center">{t('You need an active UnitySwap Profile to take part in an IFO!')}</Text>
    }
    if (publicIfoData.status === 'coming_soon') {
      return (
        <>
          <TokenSection img="/images/panther-placeholder.svg">
    ...
    if (publicIfoData.status === 'live') {
      return (
        <>
          <TokenSection img="/images/farms/ryip-bnb.svg" mb="24px">
    ...
    if (publicIfoData.status === 'finished') {
      return userPoolCharacteristics.amountTokenCommittedInLP.isEqualTo(0) ? (
        <Flex flexDirection="column" alignItems="center">
          <Image src="/images/panther-placeholder.svg" width={80} height={80} mb="16px" />
